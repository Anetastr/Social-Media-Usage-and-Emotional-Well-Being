...
# Cesta k datum
file_path = 'C:\\Users\Martin Havránek\\Desktop\\Anet projekt\\archive\\train.csv'
...
import io
# Zadej svůj API klíč a URL dle vytvořeného schématu
API_KEY = "TVŮJ_MOCKAROO_API_KLÍČ"
url = f"https://api.mockaroo.com/api/generate.csv?key={API_KEY}&count=1000&schema=TVÉ_SCHEMA_NAZEV"

try:
    response = requests.get(url)
    response.raise_for_status()

    # Načtení CSV do pandas DataFrame
    df = pd.read_csv(io.StringIO(response.text))
    print(df.head())

    # Uložení do CSV pro další použití
    df.to_csv("mockaroo_data.csv", index=False)
    print("Data uložena do 'mockaroo_data.csv'")
